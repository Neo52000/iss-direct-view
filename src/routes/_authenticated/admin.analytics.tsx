...
function AdminAnalyticsPage() {
  ...
  const chartData = [...products]
    ...
    .map((p) => ({ name: p.title.length > 22 ? p.title.slice(0, 22) + "…" : p.title, clicks: p.clicks, id: p.id }));
    ...
    <div className="mx-auto max-w-5xl px-4 py-10 md:px-6">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-3">
        ...
        <div className="iss-card p-5">
          ...
          <p className="mt-1 font-display text-3xl font-extrabold">{products.filter((p) => p.active).length}</p>
        </div>
        <div className="iss-card p-5">
          <p className="text-[11px] uppercase tracking-wider text-white/50">Meilleur produit</p>
          <p className="mt-1 font-display text-base font-bold leading-snug">{topProduct?.name ?? "—"}</p>
        ...
        {isLoading ? (
          ...
          <ResponsiveContainer width="100%" height={chartData.length * 48 + 40}>
            <BarChart data={chartData} layout="vertical" margin={{ top: 0, right: 20, bottom: 0, left: 0 }}>
              ...
              />
              <Tooltip
                contentStyle={{ background: "#0e1a2e", border: "1px solid rgba(255,255,255,.1)", borderRadius: 8, fontSize: 12 }}
      ...
      <div className="iss-card mt-6 overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {[...products].sort((a, b) => b.clicks - a.clicks).map((p, i) => (
              <tr key={p.id} className={i % 2 === 0 ? "bg-white/[0.02]" : ""}>
                <td className="px-4 py-3 font-medium">{p.title}</td>
                <td className="px-4 py-3 text-white/60">{p.category}</td>
                <td className="px-4 py-3 text-right font-display font-bold">{p.clicks.toLocaleString("fr")}</td>
                <td className="px-4 py-3 text-right">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${p.active ? "bg-[color:var(--iss-ok)]/15 text-[color:var(--iss-ok)]" : "bg-white/10 text-white/40"}`}>
